...
$OUT .= '<div class="container listado-paginado">';
	$totalCursos = $db->querySingle('SELECT COUNT(*) FROM temas WHERE IDcurso = '.$cursoData['IDcurso'].' AND ID IN (SELECT IDtema FROM videos)');

	$res = $db->query('SELECT * FROM temas WHERE IDcurso = '.$cursoData['IDcurso'].' AND ID IN (SELECT IDtema FROM videos)'.( (isset($_GET['last'])) ? ' AND orden < '.$_GET['last'] : '' ).' ORDER BY orden DESC, nombre LIMIT '.$totalPorPag);
	...
	if ($numPag > 1) {
		...
		for ($i = 1; $i <= $numPag; $i++) {
			...
			if ( ($i == $_GET['pag'])||( (!isset($_GET['pag']))&&($i == 1)) ) {
				...
				$OUT .= '<li><a href="?IDcurso='.$_GET['IDcurso'].'&pag='.$i.'&last='.($last+1).'">'.$i.'</a></li>';
			}
		}
